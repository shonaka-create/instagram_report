...
export function buildAnalysisGuidelines(metrics: Metrics): string {
  ...
  return [
    ...
    "- fail のステージが今月のボトルネック。処方の定石: 保存率未達→まとめ系・図解カルーセル強化 / ホーム率未達→ストーリーズのアンケート・Q&Aで内側の熱量を先に上げる / プロフ遷移率未達→最終画像とキャプションのCTA改善 / フォロワー転換率未達→自己紹介文(プロフィールの自己紹介)の1行目とハイライト整理を最優先TODOに",
    "- flags の警告は必ず反映する",
    "",
    "== 文章の質 ==",
