...
const sample: Report = {
  ...
  analysis: {
    ...
    executiveSummary:
      "今月の最大の課題は、プロフィールまで来た人をフォローに変えられていない点です。プロフィール遷移率は基準を超え、投稿からプロフィールへ誘導する導線は機能しています。しかしフォロワー転換率は合格ラインに届かず、プロフィール画面で「この人をフォローする理由」を提示できていません。一方、保存率トップのスタッフ紹介リールは発見タブ経由の新規到達を牽引しており、人物起点のコンテンツが勝ち筋であることは明確です。来月はバイオ1行目とハイライトの再設計を最優先に、勝ち筋であるスタッフ起点コンテンツへリソースを寄せます。",
    stageDiagnoses: {
      save_rate:
    ...
    bottleneck:
      "ファネルの穴は明確に『信頼→行動(フォロー転換)』です。認知・興味・プロフィール遷移までの3段は合格ラインを超えており、集客の入口は機能しています。にもかかわらず純増が伸びないのは、プロフィール画面が『訪問者を素通りさせている』ため。原因は投稿内容ではなくプロフィール設計側にあり、バイオ1行目が誰の何を解決するアカウントかを瞬時に伝えられていないことが最も疑わしい要因です。来月はコンテンツ制作より先に、この1画面の改善に着手すべきです。",
    contentInsight: {
      winPattern:
    ...
    nextActions: [
      {
        action:
          "プロフィールのバイオ1行目を『誰の・何の悩みを解決するアカウントか』が一読でわかる一文に書き換える",
